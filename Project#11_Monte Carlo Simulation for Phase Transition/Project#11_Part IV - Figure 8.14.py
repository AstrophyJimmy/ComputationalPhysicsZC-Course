...
Hs = np.arange(-10,10.5,0.5)
nH = np.size(Hs)
signs = [-1,1]
lss=['-','--']
for w, sign in enumerate(signs):
    for q, T in enumerate(TempRange):    
        ...
        for i,H in enumerate(Hs):    
            print("for T= "+ str(T)+" ,H= " + str(H)+" & sign="+str(sign))
            M[i]=np.mean(MontCarlo(T, H, n, ts, sign))    
            

        plt.plot(Hs ,M ,c=color[q],label='T='+str(T)+r"$\;M_i=$"+str(sign),
                 alpha=0.7,ls=lss[w])
        plt.legend()
        if w:
            plt.scatter(Hs ,M ,c=color[q].reshape(1,4),alpha=0.6,marker='o')
        else:
            plt.scatter(Hs ,M ,edgecolor=color[q].reshape(1,4),alpha=0.6,marker='o',
                    facecolor='none')
...
Title = "Figure 8.14 - Hysteresis loops"
plt.ylabel("M")
plt.xlabel("H")
plt.title(Title)
plt.grid(alpha=0.2)  
